...
const Questions = ({ questions, onHandleQuestionResponse, userResponse, language = 'en', ma = [] }) => {
    ...
    const handleCheckboxChange = (questionId, value) => {
        setFormData((prevData) => {
            let updatedValues;
            if (noneOfAboveIds.includes(value)) {
                updatedValues = [value];
            } else {
                const currentValues = prevData[questionId] || [];
            ...
            if (updatedValues.length > 0 && !noneOfAboveIds.includes(value)) {
