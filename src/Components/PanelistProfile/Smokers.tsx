...
class Smokers extends React.Component<any, any> {
    constructor(props) {
        super(props);
        this.state = {
    ...
    fetchDetails() {
        Promise.resolve()
            ...
            .then(() => ProfileManagementAPI.getOneDetails(ProfilesIds.Smokers, this.props.userId))
            .then((data) => {
                    ...
                    <Card>
                        <CardHeader color="primary">
                            <div className="d-flex align-items-center justify-content-between">
                                <h4>{this.props.language === 'hi' ? 'धूम्रपान न करने' : 'Smoker'}</h4>
                                <div>
                                    <Button
                                        onClick={() => this.create()}
                                        variant="primary"
...
const SmokersRedux = reduxForm<FormValue, any>({
    form: 'Smokers',
})(Smokers);
...
const SmokersWithRouter = withRouter(connect(mapStateToProps)(SmokersRedux));

export { SmokersWithRouter as Smokers };
